...
export default function PageExpense() {
  ...
  const titles = getTitleOfTransactionsByType(dataTransactions, "Expense");
  ...
  const clickLastMonth = () => {
    if (date) {
      ...
      if (date.length === 4) {
        navigate(`/finance/expense/${yearNum}`);
      } else if (date.length === 6) {
        navigate(`/finance/expense/${newDate}`);
      } else {
        return "";
      }
    }
    ...
    <>
      <section className="w-full">
        <Header
          title={`${
            date === "all"
              ? `Toutes les ${nameType("Expense").toLowerCase()}s`
              : date?.length === 4
                ? `${nameType("Expense")}s de ${date}`
                : `${nameType("Expense")}s de ${convertDate(date)}`
          }`}
