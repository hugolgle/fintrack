...
function OrdreInvest() {
  ...
  const orderData = data?.sort((a, b) => {
    const dateA = a.transaction[0]?.date
      ...
      : new Date(0); // Utilise une date par défaut si non définie
    const dateB = b.transaction[0]?.date
      ? new Date(b.transaction[0].date)
      : new Date(0); // Utilise une date par défaut si non définie
    return dateB - dateA; // Trie par ordre décroissant pour avoir la date la plus proche en premier
  });
